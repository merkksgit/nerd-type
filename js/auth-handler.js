...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!shouldHaveLogin) {
    console.log(
      "🔐 Auth handler skipped - no login modal expected on this page",
    );
    return;
  }
  ...
  function initializeAuthHandler() {
    ...
    if (!loginModal) {
      ...
      if (retryCount <= maxRetries) {
        console.log(
          `⏳ Login modal not found, retrying in 500ms... (${retryCount}/${maxRetries})`,
        );
        setTimeout(initializeAuthHandler, 500);
      } else {
        console.log(
          "⚠️ Login modal not found after maximum retries - auth handler disabled",
        );
      }
      return;
    }
